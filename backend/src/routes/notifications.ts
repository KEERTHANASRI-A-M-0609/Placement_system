...
router.post(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const { title, message, type, moduleId } = req.body as {
      title?: string
      message?: string
...
router.post(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const result = await notificationDispatch.dispatchEmailOnly(
      user.email,
      title ?? 'Vertex Update',
      message ?? '',
    )
...
router.post(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const result = await sendTestInactiveEmail(user.email, user.name)
    res.json(result)
  }),
)
