...
export class ReadinessEngine {
  ...
  static async calculateScore(userId: string) {
    ...
    for (const [category, score] of Object.entries(scores)) {
      const weight = (this.WEIGHTS as any)[category];
    ...
    const evidenceCount = [
      scores.DSA > 0, scores.PROJECTS > 0, scores.RESUME > 0,
      latestAssessment?.communication > 0, latestAssessment?.aptitude > 0,
  ...
  private static async calculateInterviewScore(interviews: any[]) {
  ...
  private static getMissingCategories(scores: any, assessment: any) {
    const missing = [];
